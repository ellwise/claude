...
- [ ] **CCP — Common Closure** — Flag classes that change in the same commits but live in different packages, using `git log --follow`; consolidate them. If git history is unavailable, treat heavy mutual imports across package boundaries as a proxy for co-change.
